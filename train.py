...
with tf.Session() as sess:
    ...
    # reshape将二维的句子向量变成cnn的输入（4维向量）：[batch,in_height,in_width,in_channels]
    ...
    '''
    num_classes, 6, num of the labels
    embedding_dim, 50, The dimension of the word embedding
    filter_size = [1,2,100]
    num_filters_A, 20, The number of filters in block A
    num_filters_B, 20, The number of filters in block B
    n_hidden, 150, 'number of hidden units in the fully connected layer'
    input_x1, 输入句子矩阵。shape为[batch_size,sentence_length， embed_size，1]
    input_x2, 输入句子矩阵。shape为[batch_size,sentence_length， embed_size，1]
    input_y, 输出6维的array,one-hot编码
    dropout_keep_prob = dropout比率
    '''
    # 构建MPCNN模型
